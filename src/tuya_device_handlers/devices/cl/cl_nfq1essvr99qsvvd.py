...
This device (Canisteo Smart Zebra Shades/Blinds) reports ``percent_state``
in the standard HA convention (0 = closed, 100 = open).  The default CL
mapping uses ``DPCodeInvertedPercentageWrapper`` because most Tuya
curtain/blind motors report position inverted (0 = open, 100 = closed),
which causes the position to be displayed backwards for this device.

Applying ``_InvertedIntegerTypeInformationEx`` to ``percent_state``
pre-inverts the value at the TypeInformation level, so the wrapper's own
inversion cancels out and the position is reported correctly.  The
``percent_control`` write path uses the plain ``IntegerTypeInformation`` so
commands are sent in the device's native convention.
